...
import * as userServer from '@socket/user.socket'
...
describe('EditBasicInfo', () => {
  ...
  describe('info', () => {
    it('should call updateSingleUserItemInCache', async () => {
      ...
      for (const [key, value] of Object.entries(req.body)) {
        ...
        ).toHaveBeenCalledWith(`${req.currentUser?.userId}`, key, `${value}`)
      }
  ...
  describe('social', () => {
    it('should call updateSingleUserItemInCache', async () => {
      ...
      expect(
        UserCache.prototype.updateSingleUserItemInCache
      ).toHaveBeenCalledWith(`${req.currentUser?.userId}`, 'social', req.body)
      expect(res.status).toHaveBeenCalledWith(200)
      expect(res.json).toHaveBeenCalledWith({
        message: 'Updated successfully',
      })
