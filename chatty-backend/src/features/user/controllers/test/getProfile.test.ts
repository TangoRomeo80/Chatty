...
describe('Get', () => {
  ...
  describe('all', () => {
    it('should send success json response if users in cache', async () => {
      ...
      jest
        .spyOn(UserCache.prototype, 'getUsersFromCache')
        .mockResolvedValue([existingUser])
      jest
        .spyOn(UserCache.prototype, 'getTotalUsersInCache')
        .mockResolvedValue(1)
      jest
      ...
      expect(
        FollowerCache.prototype.getFollowersFromCache
      ).toHaveBeenCalledWith(`followers:${req.currentUser!.userId}`)
    ...
    it('should send success json response if users in database', async () => {
      ...
      jest
        ...
        .mockResolvedValue([])
      jest
        .spyOn(followerService, 'getFollowerData')
        .mockResolvedValue([mockFollowerData])
  ...
  describe('profileAndPosts', () => {
    it('should send success json response if user in cache', async () => {
      ...
      jest
        ...
        .mockResolvedValue(existingUser)
      jest
        .spyOn(PostCache.prototype, 'getUserPostsFromCache')
  ...
  describe('randomUserSuggestions', () => {
    it('should send success json response if user in cache', async () => {
      ...
      const res: Response = authMockResponse()
      jest
        .spyOn(UserCache.prototype, 'getRandomUsersFromCache')
        .mockResolvedValue([existingUser])
    ...
    it('should send success json response if user in database', async () => {
      ...
      jest
        ...
        .mockResolvedValue([])
      jest
        .spyOn(userService, 'getRandomUsers')
        .mockResolvedValue([existingUser])

      await Get.prototype.randomUserSuggestions(req, res)
      expect(userService.getRandomUsers).toHaveBeenCalledWith(
        req.currentUser!.userId
      )
      expect(res.status).toHaveBeenCalledWith(200)
      expect(res.json).toHaveBeenCalledWith({
